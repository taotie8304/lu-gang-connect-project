 ...
 * 鲁港通 - 系统内容常量定义
 */
import { LangEnum, type localeType } from '../../common/i18n/type';
...
const localeSuffixMap: Partial<Record<localeType, string>> = {
  [LangEnum.en]: '_en',
  [LangEnum.zh_CN]: '_zh-CN',
  [LangEnum.ko_KR]: '_en'
};

/**
 * 鲁港通 - 根据请求语言把基准内容 key 解析为对应本地化 key（纯函数，便于单测）。
 * zh-Hant 或无对应本地化版本时回退基准 key。
 * @param baseKey 基准内容 key（如 terms_of_use，前端始终请求基准 key）
 * @param locale 请求语言（官方 getLocale(req) 结果，兜底为 en）
 * @returns 命中枚举的本地化 key，否则回退 baseKey
 */
export function resolveSystemContentKey(
  baseKey: string,
  locale: localeType
): SystemContentKeyEnum {
  const suffix = localeSuffixMap[locale];
  if (suffix) {
    const candidate = `${baseKey}${suffix}` as SystemContentKeyEnum;
    if ((Object.values(SystemContentKeyEnum) as string[]).includes(candidate)) {
      return candidate;
    }
  }
  return baseKey as SystemContentKeyEnum;
}
