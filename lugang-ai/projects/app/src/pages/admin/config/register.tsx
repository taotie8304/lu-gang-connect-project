...
const AdminRegisterConfigPage = () => {
  ...
  // 获取配置
  const { runAsync: fetchConfig } = useRequest(
    async () => {
  ...
  // 保存配置
  const { runAsync: saveConfig, loading: savingConfig } = useRequest(
  ...
  // 测试邮件发送
  const { runAsync: sendTestEmail, loading: sendingTest } = useRequest(
    async (email: string) => {
      ...
      <Modal isOpen={isTestModalOpen} onClose={onTestModalClose} isCentered>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button colorScheme="blue" onClick={handleTestEmail} isLoading={sendingTest}>
              发送测试
            </Button>
          </ModalFooter>
        </ModalContent>
