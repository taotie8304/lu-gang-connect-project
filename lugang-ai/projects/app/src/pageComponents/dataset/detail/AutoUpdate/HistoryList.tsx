...
const HistoryList = ({ collectionId }: HistoryListProps) => {
  ...
  // 鲁港通 - 加载历史记录
  const { data: historyData, loading } = useRequest2(
    async () => {
    ...
    <Box>
          ...
          <Table variant={'simple'} size={'sm'}>
            ...
            <Tbody>
              {history.map((item, index) => (
                <Tr key={index}>
                  <Td fontSize={'xs'}>{formatTime2YMDHM(item.timestamp)}</Td>
                  <Td>
                    <Badge
      ...
      {historyData && (
        <Flex mt={3} fontSize={'xs'} color={'myGray.500'} alignItems={'center'}>
          <MyIcon name={'common/clockLight'} w={'12px'} mr={1} />
          <Text>
            {t('dataset:last_check_time')}:{' '}
            {historyData.lastCheckTime
              ? formatTime2YMDHM(historyData.lastCheckTime)
              : t('common:Never')}
          </Text>
          <Text ml={4}>
            {t('dataset:last_update_time')}:{' '}
            {historyData.lastUpdateTime
              ? formatTime2YMDHM(historyData.lastUpdateTime)
              : t('common:Never')}
          </Text>
        </Flex>
      )}
