...
import type { AutoUpdateConfigType, DetectResultType } from '@/web/core/dataset/type';
...
const AutoUpdate = () => {
  ...
  // 鲁港通 - 加载配置
  const { loading: isLoadingConfig } = useRequest(
    async () => {
      if (!collectionId) return;
      const data = await getAutoUpdateConfig(collectionId);
      if (data) {
        // 合并默认值，兼容后端历史配置缺字段的情况
        setConfig((prev) => ({
          ...prev,
          ...data,
          detection: { ...prev.detection, ...(data.detection || {}) }
        }));
      }
    },
    {
  ...
  // 鲁港通 - 保存配置
  const { runAsync: onSaveConfig, loading: isSaving } = useRequest(
    async () => {
  ...
  // 鲁港通 - 手动触发更新（透出后端中文 message，区分「已更新/无变化/失败」）
  const { runAsync: onTriggerUpdate, loading: isTriggering } = useRequest(
    async () => {
      ...
      toast({
        ...
        title:
          result.message ||
          (result.success
            ? t('dataset:auto_update_triggered')
            : t('dataset:auto_update_trigger_failed'))
      });
      return result;
    },
    {
  ...
  // 鲁港通 - 识别数据集
  const { runAsync: onDetect, loading: isDetecting } = useRequest(
    async () => {
    ...
    <MyBox isLoading={isLoading} h={'100%'} py={[2, 4]}>
              ...
              <Box>
                ...
                <Select
                  ...
                  onChange={(e) => {
                    setConfig({
                      ...config,
                      fileFormat: e.target.value as AutoUpdateConfigType['fileFormat']
                    });
                  }}
                >
      ...
      {detectResult && (
        <DetectModal
          ...
          onSelectFile={(file) => {
            // 鲁港通 - 用户选择文件或 API 后，回填配置
            if (file.format === 'api' && detectResult.apiInfo) {
              // API 类型：datasetUrl 填 API 地址，并写入 api 配置（schema 无 format 字段，格式由 fileFormat 表达）
              setConfig({
                ...config,
                datasetUrl: detectResult.apiInfo.endpoint,
                fileFormat: 'api',
                api: {
              ...
              setConfig({
                ...
                fileFormat: file.format as AutoUpdateConfigType['fileFormat']
              });
            }
            onDetectModalClose();
          }}
