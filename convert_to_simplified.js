 ...
 * 鲁港通 - 将繁体使用条款转换为简体
 * 
 * 使用方法：node convert_to_simplified.js
 */
...
async function convertToSimplified() {
  ...
  try {
    ...
    // 获取繁体版本
    const traditionalDoc = await collection.findOne({ key: 'terms_of_use' });
    
    if (!traditionalDoc) {
      console.error('❌ 未找到繁体版使用条款');
      process.exit(1);
    }

    console.log('📝 找到繁体版使用条款');
    ...
    // 检查简体版本是否已存在
    const existing = await collection.findOne({ key: 'terms_of_use_zh-CN' });
    
    if (existing) {
      console.log('\n⚠️  简体版使用条款已存在，将更新内容...');
      
      const result = await collection.updateOne(
        { key: 'terms_of_use_zh-CN' },
        {
          $set: {
      ...
      console.log(`✅ 已更新简体版使用条款 (匹配: ${result.matchedCount}, 修改: ${result.modifiedCount})`);
    } else {
      console.log('\n📝 简体版使用条款不存在，将创建新记录...');
      ...
      console.log(`✅ 已创建简体版使用条款 (ID: ${result.insertedId})`);
    }

    // 验证内容
    const doc = await collection.findOne({ key: 'terms_of_use_zh-CN' });
    console.log(`\n📊 内容统计：`);
