...
- [x] 1. 品牌化资源修复
  ...
  - [x] 1.2 添加鲁港通品牌 Logo 文件
    - 创建 `/public/imgs/chat/lugang_banner.svg` (横幅 Logo)
    - 创建 `/public/imgs/chat/lugang_banner_fold.svg` (折叠版 Logo)
    - _Requirements: 1.1, 1.2, 1.5_
  - [x] 1.3 更新聊天界面 Banner 配置
...
- [x] 4. 聊天欢迎页实现
  - [x] 4.1 更新欢迎页组件
    - 修改 `src/components/core/chat/ChatContainer/ChatBox/components/home/WelcomeHomeBox.tsx`
    - 淡蓝色光晕背景效果（替代粉红色）
    - 居中问候语显示
    - _Requirements: 7.5_
  - [x] 4.2 实现问候语交替显示
...
- [x] 5. 普通用户聊天界面优化
  - [x] 5.1 隐藏管理功能入口
    ...
    - 修改 `src/components/Layout/navbarPhone.tsx`
    - 普通用户隐藏应用管理、知识库等入口
    - _Requirements: 7.1_
  - [x] 5.2 实现简化用户菜单
...
- [x] 6. Checkpoint - 界面功能验证
  - ✅ 所有界面修改代码已验证无语法错误
  - ✅ 管理员和普通用户登录路由逻辑已实现
  - ✅ 欢迎页显示和切换逻辑已实现
  - ✅ 所有资源文件（Logo、Banner）已就位
  - ✅ 登录/注册页面港话通风格改造完成
  - ✅ 快捷功能按钮已添加
  - ✅ 简化用户菜单（额度显示、充值入口）已实现

- [x] 7. One API 用户同步集成
  - [x] 7.1 创建 One API 集成服务
    - 创建 `src/service/integration/oneapi.ts`
    - 实现用户创建 API 调用
    - 实现额度查询 API 调用
    - _Requirements: 8.1, 8.3_
  - [x] 7.2 修改用户注册流程
    - 在 FastGPT 登录成功后同步创建 One API 用户（首次登录自动创建）
    - 分配初始额度
...
- [x] 11. 管理员用户管理功能
  - [x] 11.1 创建用户列表接口
    - 创建 `src/pages/api/admin/users/list.ts`
    - 支持分页和搜索
  ...
  - [x] 11.3 实现用户禁用功能
    - 创建 `src/pages/api/admin/users/status.ts`
    - 禁用/启用用户
    - 同步禁用 One API 用户
    - _Requirements: 5.3, 8.5_
  - [x] 11.4 编写用户搜索测试
    - 创建 `test/cases/lugang/user-management.test.ts`
    - **Property 6: 用户搜索准确性**
...
- [x] 12. 配置管理功能
  - [x] 12.1 创建注册配置页面
    ...
    - 创建 `src/pages/api/admin/config/register.ts`
    - 创建 `src/pages/api/admin/config/test-email.ts`
    - 邮箱注册开关和 SMTP 配置
    - _Requirements: 6.1, 6.2_
  - [x] 12.2 编写配置生效测试
    - 创建 `test/cases/lugang/config.test.ts`
    - **Property 7: 配置即时生效**
    - **Validates: Requirements 6.5**

- [x] 13. Final Checkpoint - 完整功能验证
  - ✅ 所有 153 个测试用例通过
  - ✅ 品牌化显示正确（鲁港通 Logo、淡蓝色主题）
  - ✅ 用户管理功能完成（列表、搜索、禁用/启用）
  - ✅ 配置管理功能完成（邮箱注册开关、SMTP 配置）
  - ✅ One API 集成完成（用户同步、额度查询、状态同步）
  - ✅ 完整用户流程验证（注册→登录→聊天→充值）
  - 确保所有功能正常工作
  - 验证品牌化显示正确
